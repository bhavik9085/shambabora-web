...
import { postLocationRegion, updateRegion } from '@/helpers/api-helper'
import { useMutation, useQueryClient } from '@tanstack/react-query'
import { addAlert } from '@/store/slices/elert-slice'

const formSchema = z.object({
  name: z.string().min(1, { message: 'Please enter region name' }),
})
...
const AddEditRegion = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      dispatch(
        addAlert({
          message: mode === 'edit' ? 'Region updated successfully!' : 'Region added successfully!',
  ...
  return (
    <Dialog open={true} onOpenChange={handleCancel}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {mode === 'edit' ? 'Edit Region' : 'Add Region'}
          </DialogTitle>
          <DialogDescription>
            {mode === 'edit'
